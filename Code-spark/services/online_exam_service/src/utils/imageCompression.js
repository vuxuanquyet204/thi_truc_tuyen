 ...
 */
async function compressFrameBatch(frames, quality = 0.7, options = {}) {
  ...
  for (let i = 0; i < frames.length; i += batchSize) {
    ...
    const batchPromises = batch.map(frame => 
      compressImage(frame.buffer, quality, {
        ...options,
        frameId: frame.id,
    ...
    const compressedBatch = await Promise.all(batchPromises);
    compressedFrames.push(...compressedBatch.map((buffer, index) => ({
      id: batch[index].id,
      buffer,
      timestamp: batch[index].timestamp,
      originalSize: batch[index].buffer.length,
      compressedSize: buffer.length,
      compressionRatio: buffer.length / batch[index].buffer.length
    })));
  }
...
/**
 * Smart compression with automatic quality adjustment
 */
async function smartCompress(frameBuffer, targetSize, options = {}) {
  const maxAttempts = 5;
  let quality = 0.8;
  let attempt = 0;
  
  while (attempt < maxAttempts) {
    const compressed = await compressImage(frameBuffer, quality, options);
    
    if (compressed.length <= targetSize || quality <= 0.1) {
      return {
        buffer: compressed,
        quality,
        attempts: attempt + 1,
        stats: getCompressionStats(frameBuffer, compressed)
      };
    }
  ...
  const finalCompressed = await compressImage(frameBuffer, 0.1, options);
  return {
    buffer: finalCompressed,
    quality: 0.1,
    attempts: maxAttempts,
    stats: getCompressionStats(frameBuffer, finalCompressed)
  };
}
