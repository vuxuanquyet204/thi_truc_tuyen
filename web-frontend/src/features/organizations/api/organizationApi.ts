 ...
 * Organization API
 *
 * Organization management for the organization feature.
...
const transformOrganization = (backendOrg: any): Organization => ({
	...
	shortDescription:
		backendOrg.shortDescription || backendOrg.short_description || '',
	...
	createdAt:
		backendOrg.createdAt || backendOrg.created_at || new Date().toISOString(),
	updatedAt:
		backendOrg.updatedAt || backendOrg.updated_at || new Date().toISOString(),
	lastLoginAt:
...
const transformOrganizationForm = (
	...
	if (form.isVerified !== undefined) data.is_verified = form.isVerified
